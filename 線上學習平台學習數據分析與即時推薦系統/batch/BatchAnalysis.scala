...
object BatchAnalysis {
  def main(args: Array[String]): Unit = {
    ...
    val allInfoDF: Dataset[AnswerWithRecommendations] = spark
      .read // 離線分析
      .jdbc(
      "jdbc:mysql://localhost:3306/edu?useUnicode=true&characterEncoding=utf8",
      "t_recommended",
      properties).as[AnswerWithRecommendations] //轉為.as[AnswerWithRecommendations]範型資料格式(最後一項帶有Recommendations預測推薦題目!!!)
    ...
    //TODO ===DSL
    //1.統計TOP20熱門題目--子查詢t1
    val hotTOP20: Dataset[Row] = allInfoDF
      .groupBy('question_id)
      .agg(count('question_id) as "hot_question_count")
      .orderBy('hot_question_count.desc)
      .limit(20)
    //2.將t1和原始列表t_answer關聯,得到TOP20熱門題目的推薦列表t2
    val ridAndsDS: DataFrame = hotTOP20
      .join(allInfoDF.dropDuplicates("question_id"),'question_id)
      .select("recommendations")
    //3.用Split將recommendations中的ids用","切分為數組,然後用explode將列轉為行,並記為t3,.dropDuplicates("rid") => 進行去重
    val ridsDS: Dataset[Row] = ridAndsDS
      .select(explode(split('recommendations, ",") as "rid"))
      .dropDuplicates("rid")
    ...
    val result2: Dataset[Row] = ridsAndSid
      .groupBy("subject_id")
      .agg(count("*") as "rcount")
      .orderBy('rcount.desc)
